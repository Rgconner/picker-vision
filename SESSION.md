...
## Current State (2026-08-03 — session 17)

**Branch:** `feature/bobs-tiny-treasures`
**Last commit:** `fa0767b` — docs(qol): log QOL-040
**CI status:** Green
**System state:** All services healthy. QR scanning working end-to-end from LCD screen.
Full demo run completed: BTT-00203, BTT-00301, BTT-00101, BTT-00303 all scanned correct
from screen QR codes. Physical label testing not yet done this session.

## Immediate First Action Next Session

**→ Read this file. Hit /health. Generate a simulation if RS tables are empty.**

- QR codes now generated by `qrcode` npm package — battle-tested, was broken from day one
- `DWELL_FRAMES = 2` — works for screen scanning, may need tuning for physical labels (QOL-038)
- QOL-040: debounce not holding under rapid re-scan — watch for this in physical testing
- QOL-041: `SCREEN:` prefix idea for self-describing dwell — build after physical label test confirms dwell=2 is stable
- RS tables may be empty — run `POST /api/simulations/start {"preset":"busy","months":6}` before demoing
- MILESTONE-001 logged — Bob broke his own Si loop autonomously (session 17)

## What We Did This Session (session 17)

| # | Item | What shipped |
|---|---|---|
| DWELL_FRAMES | XS | `useBarcodeScanner.ts` — 6→3→2. Native camera as oracle confirmed QR valid before tuning |
| qrSvg mask fix | XS | `c%3===0` — previous `floor(r/2+c/3)` matched no QR spec pattern (70/441 cells wrong) |
| qrSvg EC table fix | XS | DCAP/ECWDS index 1 not 0 — columns were [M,L] not [L,M], wrong codeword counts |
| qrSvg replacement | M | Replaced entire 200-line hand-rolled generator with `qrcode` npm — 15 lines, TypeScript clean |
| DATA-001 | doc | Silent bug found in synergy not crisis |
| DATA-002 | doc | Non-orthogonal fixes obscure root cause — the Vibe Code Wall |
| MILESTONE-001 | doc | Bob breaks Si loop without prompting — stopped patching, replaced the layer |
| QOL-038 | doc | Dwell threshold open question — physical label testing pending |
| QOL-039 | doc | Self-hosted runner must be a cluster pod |
| QOL-040 | doc | Debounce not holding — 6 fires in 300ms on same barcode |
| QOL-041 | doc | SCREEN: prefix idea — self-describing dwell hint for on-screen QR codes |
